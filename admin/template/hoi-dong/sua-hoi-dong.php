<?php 
	...
	function hoi_dong ($id) {
		...
		if (isset($_POST['edit_hoidong'])) {
			...
			if ($image == "") {
				$sql = "UPDATE hoi_dong SET name = '$name', sex = '$sex', position = '$position' WHERE id = $id";
			} else {
				$sql = "UPDATE hoi_dong SET name = '$name', sex = '$sex', position = '$position', image = '$image' WHERE id = $id";
			}
			
			$result = mysqli_query($conn_vn, $sql);
			echo '<script type="text/javascript">alert(\'Bạn đã sửa Chức vụ thành công.\');</script>';
		}
	}
	...
	function getHoiDong ($id) {
		global $conn_vn;
		$sql = "SELECT * FROM hoi_dong WHERE id = $id";
		$result = mysqli_query($conn_vn, $sql);
